...
*§13.5 (Import 23) supplies the fix.* A chimeric receptor that keeps the targeting determinants of the original while binding another receptor's ligands separates ligand binding from axon guidance, and the dissociation survives. That moves this item from "not worth running" to **runnable once the reagent exists** — which is a genuine change of category, not a rescue. Nobody has built the construct, and building it is a molecular problem rather than a conceptual one. The distinction matters for where it belongs: blocked on chemistry is a different list from uninterpretable in principle.
