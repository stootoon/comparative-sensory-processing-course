...
//
// Serves §12.4. The dataset account is not the only story that predicts
...
const DETAIL = {
  ...
  harder: {
    ...
    against: ['dimensionality alone cannot be it — images are far higher-dimensional and yield',
              ...
              'gustation is unordered and not stuck (§11.1), so disorder alone is not sufficient'],
  ...
  truth: {
    ...
    against: ['it is a MECHANISM for the dataset account, not an alternative to it',
              ...
              'composition as an interchange format is a route around it — §12.5'],
...
export function draw(root, values, { createDiagram }) {
  ...
  if (v === 'all') {
    ...
    n.innerHTML = `<strong>What would isolate it:</strong> ${t.isolate}` +
      `<br><br>Each rival is stated at full strength deliberately. The honest position at the end of §12.4 is that ` +
  ...
  c.textContent = 'Constructed for this course. The scoring is an argument, and §12.4 defends it in prose.';
  root.appendChild(c);
}
