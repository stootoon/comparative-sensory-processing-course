...
Possibly nothing. It would be an engineering success with no normative content: no derived coordinates for odour space, no account of why receptor tuning breadth is what it is, no statement of what the bulb computes, nothing that answers a single item in §13.7. Machine vision is the cautionary case — the labelled corpus bought recognition, and vision's <em>normative theory</em> came from somewhere else entirely, from unlabelled natural image statistics measured decades earlier by people doing something different.
