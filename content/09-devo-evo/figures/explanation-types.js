...
//
// Serves §9.1. The course has been running one explanation type — optimality —
...
export function draw(root, values, { createDiagram }) {
  ...
  n.innerHTML = f==='all'
    ...
    : `Contingency is the explanation of last resort and should be. But it is the correct explanation sometimes, ` +
      ...
      `predicts it. Holding that row in mind is what keeps §9.4's convergence argument honest.`;
  root.appendChild(n);
  const c=document.createElement('p'); c.className='x-figure-credit';
  c.textContent='Compiled from the §11 sections across the course. "How settled" is this course’s judgement against §0.2, not a literature consensus.';
  root.appendChild(c);
}
