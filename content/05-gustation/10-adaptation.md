...
Taste's version of the requirement is the harder one to state, and it is a direct consequence of the labelled lines of §5.4. A replacement cell in a bud of roughly a hundred must both commit to one of the five qualities and find the afferent already carrying that quality — a new sweet cell onto a sweet fibre, a new bitter cell onto a bitter fibre [@kandel2021]. Get the pairing wrong and the animal does not lose acuity, it gets the *sign* wrong: sugar routed onto the aversive line. A combinatorial code degrades gracefully under miswiring; a labelled line does not, which is a real cost of the architecture and is rarely counted against it.
