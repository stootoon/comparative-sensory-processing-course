...
//
// Serves §10.3. Two things this figure has to make visible at once. First, the
...
export function draw(root, values, { createPlot }) {
  ...
  if(mode==='eco'){
    ...
    note(root,
      ...
      `is substantially set by genomic dynamics that the coding problem does not see. §10.3 argues you cannot ` +
  ...
  note(root, isFrac
    ...
    : `<strong>Nearly three orders of magnitude, across animals that all have to find food and avoid predators by ` +
      ...
      `§10.3 treats "the repertoire is sized for the coding problem" as a hypothesis rather than a premise.`);
}
