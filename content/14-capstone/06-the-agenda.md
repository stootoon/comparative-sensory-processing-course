...
*A requirement added by §14.5 (Import 24):* the genomic alternative must be measured in the **same** experiment, not invoked afterwards in the discussion. Extract cluster membership and subfamily size from the same genomes and analyse the result as a model comparison between coding demand, birth-and-death history, and both. Without that, a null is a fact rather than a finding — it says one optimality argument failed to transfer and cannot say why. With it, the informative quantity becomes the residual variance in abundance once genomic history is regressed out, which is what the coding claim is actually about and which nobody has isolated.

*Cost:* very low — largely a data-analysis project on published data, and the genomic covariates are already in the sequence databases.
...
*§14.5 (Import 23) supplies the fix.* A chimeric receptor that keeps the targeting determinants of the original while binding another receptor's ligands separates ligand binding from axon guidance, and the dissociation survives. That moves this item from "not worth running" to **runnable once the reagent exists** — which is a genuine change of category, not a rescue. Nobody has built the construct, and building it is a molecular problem rather than a conceptual one. The distinction matters for where it belongs: blocked on chemistry is a different list from uninterpretable in principle.
...
Two kinds of thing came out of the preceding sections, and they should not be filed together.

**The eliminations are settled, and they are stated as such.** Taste is unordered, low-dimensional and not stuck, so **disorder is not sufficient for olfaction's difficulty** — one case with the property and without the consequence is all a sufficiency refutation needs. Touch and proprioception route around the thalamus and neither is ancient, so **antiquity is not required for a thalamic bypass**. Nociception is ordered, low-dimensional and still has no normative account, so **neither ordering nor low dimensionality is sufficient for a theory to exist**. And because an unordered array supplies no metric, it supplies no convolutional prior, so **intrinsic difficulty and data requirement compound rather than compete** — a system with no metric needs *more* data to learn what a metric would have given for free, which means the candidate explanations of olfaction's position are not rivals in the way they are usually posed.

Those four hold regardless of how the rest resolves. What follows does not.

**The open questions are open, and you now have the evidence to weigh them.** §0.0 listed four candidate explanations of why olfaction has no theory of the kind the retina has — the unmeasured ensemble, intrinsic difficulty from dimensionality and the missing metric, mixtures failing to superpose at the receptor so the linear machinery does not apply, and the field simply being younger and smaller. Each of the following bears on which of them is doing work.

**1. Is olfaction's difficulty about dimensionality, or about something dimensionality is standing in for?** The elimination removes disorder as a sufficient cause; it does not install dimensionality in its place. §12.1's audit lands on an interaction — disorder is nearly costless at five dimensions, where every distinction can be wired individually, and expensive at a thousand, where exploiting structure is the only way to avoid per-connection cost — but the interaction has no independent test case in this material. And ~1000 is a receptor count, not a measured intrinsic dimension; proprioception's twenty nominal against three effective is the standing warning. Non-stationarity and the sensor–signal mismatch are live alternatives that survive the same gustation dissociation. *(Import 11, §12.1, §12.2)*

**2. Where are olfaction's tractable normative problems, and why are they there?** The plume ensemble is measured, analytically characterised and under-exploited; the chemical ensemble is not measured. That much is a fact about the field. Two readings of it point at different work. On the data reading, tractability follows the dataset, and the prescription is to build the missing one. On the objective reading, the temporal problems are tractable because they arrive with a stated estimation problem attached — source separation, distance from intermittency, timing against a deadline — and a database would not confer that on the chemical side. Gap 2 in §14.7 is the version of this question you can act on without new measurements. *(Import 15)*

**3. Is the missing relay computational or evolutionary?** Antiquity is refuted as a *general* account by the bypasses, and that part is secure. The replacement — deadline pressure plus a consumer wanting speed rather than re-registration — is not established: the quantitative comparison that would test it (item 8) has not been run, and §12.4 finds the deadline conjunct fails on olfaction's own numbers. What survives is narrow and consistent with several stories. *(Import 10)*

Each of these is decidable, and the experiments above are what would decide them. That is the difference between this list and a set of conclusions.
