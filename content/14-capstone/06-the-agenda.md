...
**1. Disorder alone is not the culprit — the honest form is an interaction.** Taste is unordered and easy, so disorder is not sufficient for difficulty, and the metric hunt that has absorbed a great deal of effort is at best addressing half the variable. But §12.1's audit will not support the stronger slogan either: disorder is nearly costless at five dimensions, where every distinction can be wired individually, and expensive at a thousand, where exploiting structure is the only way to avoid per-connection cost. **Neither factor alone reproduces olfaction's profile.** The secure part is the elimination; "dimensionality is the operative cause" is an inference. *(Import 11, §12.1)*

Note that this is the weakest of the three, and §14.8 asks you to argue it is wrong. The correlations in §0.0's figure cut against the slogan directly — across the datasheet, ordered-array predicts theoretical maturity slightly *better* than dimensionality does, and the case for blaming dimensionality rests on gustation being an outlier the coefficient averages away rather than on the correlation itself.
