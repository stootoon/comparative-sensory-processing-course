...
A neuron has a limited dynamic range: an auditory nerve fibre's rate–level function typically covers 30–40 dB before saturating, while the audible range spans 120 dB. Two distinct solutions are already in place before this section's result, and they are worth separating from it. The cochlear amplifier compresses mechanically, folding 120 dB of pressure into two or three orders of magnitude of membrane displacement (§2.3). The auditory nerve then tiles what is left with fibres of *staggered threshold* — the high-spontaneous-rate fibres saturating by about 30 dB SPL, the low-spontaneous ones still responding gradedly above 100 dB (§2.4). Between them, a population of narrow-range channels covers a range no one of them could.

Neither is adaptive. Both are fixed allocations, good for the range of levels the species encounters rather than for the levels in the room right now. The efficient-coding prescription from §0.1 asks for more than that: the transfer function should match the *cumulative distribution of inputs actually encountered* — Laughlin's rule.
...
- **Hair-cell adaptation**, which is mechanical rather than synaptic and is easy to mis-assign. During a maintained bundle deflection the transduction channels reclose and the whole displacement–response curve *slides* along the displacement axis, by about 80% of the imposed offset, without changing shape [@kandel2021]. Two processes contribute: a fast one, under a millisecond, in which Ca²⁺ entering through the channel favours its closure, and a slow one, tens of milliseconds, in which a myosin motor climbs or slides along the stereociliary actin core to reset gating-spring tension. The consequence is a high-pass filter with a very useful property — the cell stays maximally sensitive to small rapid deflections while rejecting static offsets a million times larger. This is Laughlin's rule again, implemented in a motor protein rather than in a spike rate, at the very first stage.
- **Auditory nerve adaptation**, distinct from the above: a brisk onset response decaying to a plateau over a few tens of milliseconds, and a matching silent period after the sound stops. This one is largely synaptic depression at the ribbon synapse.
- **Efferent gain control.** The medial olivocochlear neurons feed back onto outer hair cells, mostly contralaterally, through an unusual nicotinic receptor built from α9 and α10 subunits; Ca²⁺ entering through it opens K⁺ channels, hyperpolarises the cell and so turns the amplifier down [@kandel2021]. A separate lateral olivocochlear group targets the *afferent fibres* of inner hair cells ipsilaterally, and appears to balance the excitability of the two ears against each other — which is a gain control whose purpose is not dynamic range at all, but protecting the binaural comparison of §2.5 from drifting. Compare cortical feedback to the olfactory bulb: in both cases a central structure adjusts the gain of the first stage, and in both cases the function is debated.
