...
<x-figure src="content/13-artificial/figures/sensor-drift.js"
...
The second is a supervised learning problem with no training set. That is a strikingly specific way to be stuck, and it is check 3 from §13.1's list: the stall is at the input representation and at the input–output pairs, not at the classifier.
...
Two absences follow, and they are the engineering statement of §14.7's Gaps 1 and 4.
...
The nearest thing the field has had to a benchmark episode is instructive on both counts. A published psychophysical dataset — single molecules, presented at fixed concentrations, rated by human panellists on verbal descriptor scales — was used as the basis of an open prediction challenge in which many groups predicted perceptual ratings from molecular structure. It produced measurable, rapid progress, exactly as the benchmark argument predicts. And the scope of it is the argument's other half: **single molecules, at fixed concentration, described in words.** That is not the natural ensemble. It is §14.7's Gap 3 — the standard stimulus cannot reveal what is at issue — reappearing in engineering, and it means the one place machine olfaction has had a benchmark is the one place it has moved.
