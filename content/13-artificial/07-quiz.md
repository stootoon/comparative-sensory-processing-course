...
lede: Eight questions on a comparison that supports several explanations at once and settles none of them. Several ask you to attack whichever candidate you found most persuasive, which is the skill the module was written to develop.
estimatedMinutes: 26
---

This module laid out five candidate explanations and then spent a section refusing to pick one. The quiz continues that. Most of the questions have a defensible answer other than the intended one, and the distractor feedback says which assumption would make each of them right.
...
<script type="application/json">
{
  ...
  "options": [
    ...
    },
    {
      ...
      "correct": false,
      "feedback": "Correct under the assumption that the most salient difference is the operative one, and §13.4 gives this candidate a full hearing rather than treating it as a mistake — in its strongest form, an odour corpus is not a redistributable object at all, which would account for the engineering gap entirely. What it cannot do is explain the biological half of the asymmetry: animals have excellent chemical sensors and olfactory theory lagged anyway. That is a reason to prefer a candidate covering both, but preferring is not concluding, and a two-cause world is perfectly possible."
    }
  ]
}
</script>
...
<script type="application/json">
{
  ...
  "contentRev": 1,
  "points": 1,
  "prompt": "Which observation in this module does the most to separate §13.4's five candidate explanations from one another, and why?",
  "options": [
    {
      ...
      "correct": true,
      "feedback": "This is the module's most discriminating observation, and its virtue is that it is a within-subject comparison: the animal, the receptors, the chemistry, the sensor hardware and the commercial pull are all constant across the split. It eliminates three of the five candidates — hardware, effort and raw dimensionality all predict olfaction failing uniformly. It does not get you to one. The plume problem has a metric (time) and the chemical problem has none, so the missing-metric candidate predicts the same split as the unmeasured-ensemble candidate does, by a different route, and the two are entangled anyway: a sub-problem with a metric is a sub-problem whose ensemble is cheap to measure. Note also that this evidence arrives from literatures assembled for unrelated reasons, which is criterion 4 of §0.2 doing real work."
    },
    {
    ...
    },
    {
      ...
      "correct": false,
      "feedback": "Correct under the assumption that locating the stall is what discriminates, and this is genuinely strong — it is the risky check §13.1 stated in advance and §13.3 confirmed. But 'the stall is at the input' is compatible with at least three of the five candidates — the input being unobtainable for hardware reasons, the input being unlearnable without a metric, or the input simply being where the unspent effort would have gone. It localises without adjudicating."
    },
    {
...
<script type="application/json">
{
  ...
  "partialCredit": true,
  "shuffleSeed": 31,
  "prompt": "Rank these five ways of separating §13.4's candidate explanations by value for money — best ratio of discriminating power to cost, first. §13.4 lists them; the question is whether you can reconstruct the ordering from the arguments.",
  "items": [
  ...
  "modelAnswerNote": "The ordering is about cost as much as power, and two of the top three cost almost nothing.\n\n**(b) first.** It is free, it uses literatures that already exist, and it holds hardware, chemistry, dimensionality and commercial pull constant while varying what has been measured. It knocks out three of the five candidates at zero cost. What it cannot do is separate the two it leaves — the unmeasured ensemble and the missing metric both predict the split — and it is worth being clear about that limit while still ranking it first, because free plus partly discriminating beats expensive plus partly discriminating.\n\n**(c) second.** Also nearly free — it is a literature search — and it addresses the module's own worst failure, which is that criterion 1 is not met: the sample of engineered domains was chosen with the conclusion known, and only the diagonal cells were examined. A domain with good hardware, strong pull and no corpus would bear on the hardware and effort candidates simultaneously.\n\n**(a) third.** Cheap, affordable now, and the only item on the list that separates the two candidates (b) leaves standing — if a learner cannot recover source identity from *exact*, noise-free composition, the difficulty is in the structure of the problem and not in the measurement of it. It ranks below (b) and (c) only because it needs new measurements, however few. Both outcomes are informative, which is what makes it good value.\n\n**(d) fourth.** The real intervention, and expensive. It is what would bear decisively on the hardware candidate, and it is a multi-year effort even in its cheap unlabelled form.\n\n**(e) last.** Most expensive, and the least under anyone's control — drift-free standardised chemical sensing has resisted decades of materials work. It is being run slowly by instrumentation improvement whether anyone chooses it or not.\n\nA reasonable person could put (a) first on the grounds that separating the two survivors matters more than eliminating three also-rans. The indefensible answer is (e) first."
}
</script>
</x-order>
...
<script type="application/json">
{
  ...
  "contentRev": 1,
  "points": 1,
  "prompt": "§13.5 argues that Gap 1 is really two objects. Which version should a theorist who wants a normative account of olfactory coding ask for, and why is the answer encouraging?",
  "options": [
    {
      ...
      "correct": true,
      "feedback": "And the encouraging part is the cost. The labelling is where nearly all the expense sits, and the theory does not need it — exactly as a natural image database is a collection of photographs rather than a collection of judgements. §14.7 describes Gap 1 as a heroic decade-long effort, which fits the device version; the theory version is a smaller object that could be bootstrapped from compositional data atmospheric chemistry, food science and entomology are already generating for their own reasons. §13.5 adds one component the field's accounting omits: a receptor forward model, without which the ensemble cannot be pushed through the array."
    },
    {
      ...
      "correct": false,
      "feedback": "Correct under the assumption that the explanandum is perception, which is a defensible position and the one most existing odour datasets embody. But it is the expensive object, it is limited to stimuli a panel can be presented with — in practice single molecules at fixed concentration, which is §14.7's Gap 3 — and it presupposes the answer to what the code is for. §12.5 and §14.7 both argue the dominant consumer may be navigation, in which case perceptual ratings characterise a side effect."
    },
    {
...
The next module is the capstone, and this one arrives at it with a comparison rather than a conclusion — material the capstone should weigh rather than adopt.

<strong>An elimination, and it is the module's most durable product.</strong> The asymmetry between olfaction and the other classical modalities is not specifically biological — it reproduces in systems with no metabolism, no evolution, no development and no anatomy. Every account of olfaction's theoretical backwardness that appeals to those constraints is thereby subtracted from being sufficient. §14.7 should read this alongside the taste and spinocerebellar eliminations, because it has the same logical form and a wider reach.

<strong>A shorter list of candidates, and two corrections to how Gap 1 is stated.</strong> The engineering record leaves Gap 1 standing as a candidate for the binding constraint alongside the missing metric, and it eliminates hardware, effort and raw dimensionality from the within-olfaction comparison. It also corrects the accounting twice: Gap 1 is two objects of very different cost, and it has a component — the receptor forward model — that the field's statement of it omits. The capstone should carry the elimination and the corrections; it should not carry a winner, because this module does not produce one.

<strong>An objection the capstone must answer.</strong> Datasets buy engineering success and theoretical understanding separately. A large corpus could produce a working artificial nose that answers not one question in §14.7, and vision is the cautionary case, since its normative theory came from a different dataset than its recognition performance did. "Build the database" is not automatically a purchase of understanding, and the research agenda should say which good it is buying.

<strong>And a warning about the method.</strong> §13.6 scores this module against the five criteria and it fails criterion one — the sample of engineered domains was chosen by the author with the conclusion known, and only the confirming cells were examined. That is the same failure the module attributes to olfactory theory. §14.7's closing note argues the comparative method over-produces plausible questions and under-weights what has no analogue; this module is an instance of both the method's strength and its characteristic vice, and should be read with that in view.
</x-callout>
