...
//
// Serves §13.5. The section's argument is that the pieces are separable and
...
export function draw(root, values, { createDiagram }) {
  ...
  n.innerHTML = b === 'both'
    ...
    ? `<strong>This is the section's punchline.</strong> The components a normative theory needs are cheaper than the ` +
      ...
      `<br><br>Which reframes §14.7's Gap 1 in a useful way: the course has been describing the missing database as ` +
    ...
    : `A working device needs the labels, and the labels are where the cost is — the conditional p(y|x) rather than ` +
      ...
      `one §13.4 treats as a live rival explanation of the whole gap.` +
      `<br><br>Worth noticing: a system built from these rows alone could work beautifully and yield <em>no</em> ` +
      `normative account whatever. That is the objection in §13.4's callout, expressed as a purchasing decision.`;
  root.appendChild(n);
