...
//
// Serves §13.1. The module's whole argument rests on an inference from
...
export function draw(root, values, { createDiagram }) {
  ...
  n.innerHTML = f === 'all'
    ? `<strong>The rungs are not equally sound, and this module deliberately stands on only three of them.</strong> ` +
      ...
      `worth exactly as much as its list of ruled-out alternatives. §13.4 spends itself on that list.`
    : f === 'succ'
    ? `Success is <strong>over-determined</strong>. Many different systems compute the same function, so a working ` +
      ...
      `same structure, the task is doing the work. That is the form of the CNN / visual-cortex argument in §13.2, ` +
    ...
    : `Failure is the <strong>informative direction here, and the fragile one</strong>. It cannot show the problem ` +
      ...
      `The remaining risk is that the effort was never serious, which is the commercial-pull objection §13.4 ` +
